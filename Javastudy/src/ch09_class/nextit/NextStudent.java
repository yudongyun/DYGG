...
public class NextStudent {
	...
	// 생략된 상태로 존재한다.
	// public NextStudent() {} // 이런식으로 생략된 상태로 존재했던거임
	// 만약에 생성자를 따로 만들어주었다면 생략된 상태로 존재했던게 제거된다.
	// 기본 생성자를 사용을 위해 따로 만들어준다.
	...
	public NextStudent(String name, int age) {
		this(name, age, 1, 0); // 이렇게 해도된다. this 는 생성자를 의미
		this.name = name;
		this.age = age;
		// level과 exp를 입력받지 않았다면 그냥 값을 박아줌
		// 기본값 설정
		this.level = 1;
		this.exp = 0;
	}
