...
public class CollectionSet {
	...
	public static void main(String[] args) {
		...
		stuSet.add("유동윤");
		stuSet.add("홍민경");
		System.out.println(stuSet); // 주머니에 값을 담는느낌이라서 순서의 개념이 없음
		
		// 중복된 값이 추가되지는 않는다.
		stuSet.add("박진영"); // 에러는 아님
		...
		// .contains(값)
		// 해당 값이 Set안에 존재하면 true, 없으면 false
		...
		// Set은 집합을 쓰는 경우가 아니라면 잘 안쓴다.
		// 그나마 Set을 사용한다면 Set의 중복을 허용하지
		// 않는 특징을 이용하는 경우이다.
		// 배열 꾸져서 안씀. String.split -> 배열로 리턴
		
		ArrayList<Integer> intList = new ArrayList<>();
		
		// 15부터 30까지의 랜덤 숫자를 10개 담기
		// (int)(Math.random) + 
		// 1 ~ 45 -> (int)(Math.random() * 45 ) + 1;
		// 0 ~ 44 -> (int)(Math.random() * 44 );
